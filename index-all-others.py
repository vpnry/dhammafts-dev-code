...
def runIndexer(inDir, extStr, dbname):
    ...
    for path in paths:
        with open(path, "r") as fc:
            ...
            text = text.strip()
            text = text.split("\n")
            tubleList = [(path, t.strip()) for t in text if len(t.strip()) > 3]
...
# runIndexer("dict", "*", "dict.sqlite3")
# runIndexer("pa", "*", "patxt.sqlite3")
# runIndexer("pn", "*", "pn.sqlite3")
runIndexer("vi", "*", "ttpv_budsas.net.sqlite3")

...
def tika2text(inDir, outDir, frExt, toExt=".txt", fixBrokenLine=True):
    """
    ...
    for url in files:
        ...
        if (fixBrokenLine):
            ...
            content = re.sub(r'([\s]*150815[\s]*){1,}', r' ', content)
    ...
    print('-----------------------------------------')
    print('Input file total: \033[0;32m' + str(len(files)) + '\033[0m files.')
    print('\033[0;32mDone\033[0m, processes total: \033[0;32m' +
          str(i) + '\033[0m files.')
    print('Process time: ' + str(time.process_time()) + ' seconds')
    print('-----------------------------------------')


tika2text("pSearchLib", "pSearchLib_txt", "*", "txt", True)
